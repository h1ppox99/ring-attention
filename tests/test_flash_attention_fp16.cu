/// @file
/// Correctness tests for the FP16 / Tensor-Core FlashAttention kernel.
/// Mirrors test_flash_attention.cu but with a 1e-2 tolerance to account for
/// FP16 storage of Q/K/V/P and Tensor Core round-off.
...
int run_case(const AttentionShape& s, bool causal, std::uint32_t seed, const char* tag) {
  ...
  launch_flash_attention_fp16(dq.data(), dk.data(), dv.data(), dout.data(), s, causal);
...
int main() {
  ...
  // head_dim = 64.
  rc |= run_case({1, 1, 32, 32, 64}, false, 1u, "fp16 d=64 32x32 non-causal");
  rc |= run_case({1, 1, 32, 32, 64}, true, 2u, "fp16 d=64 32x32 causal");
  rc |= run_case({2, 4, 128, 128, 64}, false, 3u, "fp16 d=64 128x128 non-causal");
  rc |= run_case({2, 4, 128, 128, 64}, true, 4u, "fp16 d=64 128x128 causal");

  // Ragged: Sq=33 doesn't divide BR=16; Sk=47 doesn't divide BC=16.
  rc |= run_case({1, 2, 33, 47, 64}, false, 5u, "fp16 d=64 33x47 non-causal (ragged)");
  rc |= run_case({1, 2, 33, 47, 64}, true, 6u, "fp16 d=64 33x47 causal (ragged)");

  // Sq != Sk causal alignment.
  rc |= run_case({1, 1, 16, 48, 64}, true, 7u, "fp16 d=64 Sq<Sk causal");

  // head_dim = 32.
  rc |= run_case({1, 2, 64, 64, 32}, true, 8u, "fp16 d=32 64x64 causal");
