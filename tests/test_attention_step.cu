...
int run_ring(const AttentionShape& full, int num_chunks, bool causal, std::uint32_t seed,
  ...
  for (int p = 0; p < num_chunks; ++p) {
    ...
    // Pack chunk: for each (b, kv_h), copy chunk_k rows starting at offset k_off.
...
int main() {
  ...
  // GQA: 8 Q heads, 2 KV heads (4:1 ratio).
  {
    AttentionShape s{1, 8, 64, 64, 64};
    s.kv_heads = 2;
    rc |= run_ring(s, 2, false, 11u, "GQA H=8 kv=2 P=2 non-causal");
    rc |= run_ring(s, 2, true, 12u, "GQA H=8 kv=2 P=2 causal");
  }
  // MQA: 4 Q heads, 1 KV head.
  {
    AttentionShape s{1, 4, 64, 64, 64};
    s.kv_heads = 1;
    rc |= run_ring(s, 4, false, 13u, "MQA H=4 kv=1 P=4 non-causal");
    rc |= run_ring(s, 4, true, 14u, "MQA H=4 kv=1 P=4 causal");
  }
