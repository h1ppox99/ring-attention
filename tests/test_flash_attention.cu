...
int main() {
  ...
  // GQA: 8 Q heads, 2 KV heads.
  {
    ...
    s.kv_heads = 2;
    rc |= run_case(s, false, 11u, "GQA H=8 kv=2 non-causal");
    rc |= run_case(s, true, 12u, "GQA H=8 kv=2 causal");
  }
  // MQA: 4 Q heads, 1 KV head.
  {
    AttentionShape s{1, 4, 64, 64, 64};
    s.kv_heads = 1;
    rc |= run_case(s, false, 13u, "MQA H=4 kv=1 non-causal");
    rc |= run_case(s, true, 14u, "MQA H=4 kv=1 causal");
  }

  // head_dim = 256.
  rc |= run_case({1, 1, 32, 64, 256}, false, 11u, "d=256 32x64 non-causal");
  rc |= run_case({1, 1, 64, 64, 256}, true, 12u, "d=256 64x64 causal");
