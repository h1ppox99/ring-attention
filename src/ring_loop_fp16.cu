...
#ifdef RING_USE_NCCL
        ncclGroupStart();
        NCCL_CHECK(ncclSend(K_cur, local_elem, ncclHalf, next_rank, nccl_comm, 0));
        NCCL_CHECK(ncclRecv(K_recv, local_elem, ncclHalf, prev_rank, nccl_comm, 0));
        NCCL_CHECK(ncclSend(V_cur, local_elem, ncclHalf, next_rank, nccl_comm, 0));
        NCCL_CHECK(ncclRecv(V_recv, local_elem, ncclHalf, prev_rank, nccl_comm, 0));
  ...
  auto one_pass = [&]() -> std::tuple<double, double, double> {
    ...
    for (int step = 0; step < P; ++step) {
      ...
      if (step < P - 1) {
        ...
        ncclGroupStart();
        NCCL_CHECK(ncclSend(K_cur, local_elem, ncclHalf, next_rank, nccl_comm, stream_copy));
        NCCL_CHECK(ncclRecv(K_recv, local_elem, ncclHalf, prev_rank, nccl_comm, stream_copy));
        NCCL_CHECK(ncclSend(V_cur, local_elem, ncclHalf, next_rank, nccl_comm, stream_copy));
        NCCL_CHECK(ncclRecv(V_recv, local_elem, ncclHalf, prev_rank, nccl_comm, stream_copy));
        NCCL_CHECK(ncclGroupEnd());
