...
**What this repo did.**  `launch_attention_step` had no bias argument.  `attention_step.cu`
computed raw dot products and scales, with no additive term beyond the causal mask.

**What was done.**  Added an optional `const float* attn_bias` parameter (device pointer,
shape `[B, H, Sq, Sk]`, nullptr = disabled) to `launch_attention_step` and
`launch_attention_step_fp16`.  Inside `attention_step_kernel`, after the dot-product and
scale, the kernel adds `attn_bias[(b*H+h)*Sq*Sk + i_local*Sk + j_local]` to each visible
score before the max-reduction.  The FP16 kernel applies the bias (stored as FP32) inside
the per-row online-softmax section, after the WMMA matmul writes to `S_f`.  Callers
pre-slice the bias to the current chunk window before passing it in; the ring loop itself
needs no other changes.  Tested in `tests/test_attention_step_bias_segs.cu` (6 cases:
batched, causal/non-causal, D ∈ {32, 64, 128}).
...
**What this repo did.**  No segment-ID concept; all tokens in a single batch-head could
attend to each other (modulo the causal mask).

**What was done.**  Added optional `const int* segment_ids` (shape `[B, S_global]`,
nullptr = disabled) and `int seq_global` parameters to `launch_attention_step` and
`launch_attention_step_fp16`.  In `attention_step_kernel`, the visibility predicate is
extended: if `segment_ids` is non-null and `segment_ids[b*seq_global + i_global] !=
segment_ids[b*seq_global + j_global]`, the score for that (query, key) pair is forced to
`-INFINITY` before max-reduction (cross-segment pairs are masked out entirely, never reach
the softmax exponential, and contribute zero to the output).  The FP16 kernel applies the
same check inside the per-row online-softmax section where `S_f` is post-processed.
Callers pass the full `[B, S_global]` array unchanged at every ring step; the kernel uses
`q_offset + i_local` and `k_offset + j_local_in_chunk` as global indices, so no slicing is
needed.  Tested alongside the bias feature in `tests/test_attention_step_bias_segs.cu`
(6 cases: batched, causal/non-causal, D ∈ {32, 64, 128}).
...
| Attention bias | Yes | **Yes** (`attn_bias` param, caller pre-slices) |
| Segment IDs | Yes | **Yes** (`segment_ids` + `seq_global` params) |
| Dropout | Yes | **No** |
